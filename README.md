...
src/js/
  ...
  content.js             führt alle Daten zusammen; legt außerdem fest,
                         welche Wörter mit „Welches Bild passt?" abgefragt
                         werden dürfen (siehe unten)
  store.js               Fortschrittsspeicher
  srs.js                 verteiltes Wiederholen
  speech.js              Aussprache
  pics.js                setzt an Stelle jedes Emojis sein Bild
  ui.js                  kleine DOM-Helfer
src/img/                 787 Bilder – zu jedem Emoji des Kurses eines
  list.json              ihre Namen, damit der Service Worker sie vorlädt
src/data/
...
tools/
  ...
  make-img-list.mjs      schreibt src/img/list.json neu
...
### Wann ein Wort als Bild abgefragt wird

Jedes Wort hat ein Bild. Auf der Wortkarte und in den Listen steht es immer —
als Merkhilfe. Abgefragt („Welches Bild passt?") wird es aber nur, wenn es das
Wort auch wirklich zeigt. `content.js` prüft dafür zwei Dinge:

1. Das Emoji steht für höchstens zwei Wörter im ganzen Kurs. Sammelbilder wie
   🎯, 🔢 oder ⚖️, die auf zwanzig Wörter verteilt sind, sagen über das einzelne
   Wort nichts aus.
2. Die `id` steht nicht in der Liste `NO_PICTURE`. Dort stehen Wörter, für die
   es zwar ein seltenes, aber kein sprechendes Emoji gibt — Redewendungen,
   Füllwörter, und die Fälle, in denen das Bild eigentlich dem anderen Wort
   gehört (🍋 ist die *Zitrone*, nicht *Sizilien*).

Ablenker in der Bilderfrage kommen ebenfalls nur aus diesem Vorrat, und jedes
Bild taucht pro Frage höchstens einmal auf. Wörter ohne Bild werden dafür
öfter über Übersetzung und Aussprache geübt — die Lektion wird also nicht
kürzer, nur eindeutiger.
