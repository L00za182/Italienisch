...
function exPickIt(word, pool) {
  ...
  return {
    ...
    prompt: word.de, image: word.img, answerSpeak: word.it,
...
/**
 * „Welches Bild passt?" – nur für Wörter, die ihr Emoji wirklich zeigt.
 * Sonst (und wenn zu wenige eindeutige Ablenker da sind) `null`; die
 * Aufrufer setzen dann eine andere Übung ein.
 */
function exPickImage(word, pool) {
  if (!word.pic) return null;
  const dist = imageDistractors(word, pool, 3);
  if (dist.length < 3) return null;
  const opts = shuffle([word, ...dist]);
  return {
...
function exDrill(drill, grammarId) {
  ...
  return {
    ...
    question: 'Ergänze:',
    prompt: drill.q,
    answerSpeak: solved(drill),
    options: opts
  };
}

/** Der Drill-Satz mit eingesetzter Lösung – so wird er vorgelesen. */
function solved(drill) {
  return drill.q.replace('___', drill.a).replace(/s*→s*/g, ', ').trim();
}
 ...
 * Zeigt das Emoji das Wort nicht eindeutig, tritt an die Stelle der
 * Bilderfrage die jeweils andere der beiden übrigen Übungen.
 */
function drillsForNew(word, pool) {
  const set = [exPickDe(word, pool)];
  const img = exPickImage(word, pool);
  const listenFirst = Math.random() < 0.5;
  if (img) {
    set.push(img);
    set.push(listenFirst ? exPickIt(word, pool) : exListen(word, pool));
  } else {
    set.push(listenFirst ? exPickIt(word, pool) : exListen(word, pool));
    set.push(listenFirst ? exListen(word, pool) : exPickIt(word, pool));
  }
  return set;
}
...
function drillForReview(word, card, pool) {
  ...
  return exPickImage(word, pool) || exPickDe(word, pool);
}
